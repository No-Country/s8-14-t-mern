...
import { CustomFileTypeI } from '../interfaces/user.interface'
...
const postCardCtrl = async (req: Request, res: Response) => {
  try {
    ...
    const imageCardFile = req.files.image as CustomFileTypeI
...
const getCardCtrl = async (_req: Request, res: Response) => {
  try {
    ...
    if (error instanceof Error)
      res.status(400).json({ error: 'Does not exist card' })
  }
}
...
export { findCardByType, getCardCtrl, postCardCtrl }
